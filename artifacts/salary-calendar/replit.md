...
- Routing profile (`lib/routing.ts`): runtime-switchable `bike` / `car`. Bike uses `routing.openstreetmap.de/routed-bike` (allows cycle paths and many footways — important for e-bike couriers). Car uses `router.project-osrm.org`. `useRoute` and `useDistanceMatrix` re-key on profile and refetch on switch. Selection persisted in `localStorage` under `salary-calendar:routing-profile:v1` and restored in `main.tsx` before the first fetch. UI: cycling chip in MapView header (🚲 / 🚗). Legacy `foot` value migrates to `bike` on load.
- GPS simulator (`components/DriveMode.tsx`): tap "СИМ" in the drive-mode toolbar to disable real GPS and synthesize positions along the active route. Speed presets 10/30/60/120 km/h, ▶/⏸ to advance, "GPS" button to drop back to the device GPS. While the sim is active the completion modal auto-submits after 3s (long enough for the green "ВЫ ПРИБЫЛИ" banner to register) using the per-order rate (or ₽100 fallback), so a full wave (delivery legs → arrival → next stop → return-to-depot → finished) can be dry-run end-to-end.
- Course-up rotation (`components/DeliveryMap.tsx` + `DriveMode`): tap "↥" in the drive-mode top bar to lock the map's up-direction to the courier's heading. DeliveryMap accepts a `bearing` prop; when set, the entire map is wrapped in an oversized (≈142%) rotated container so the rotated viewport always covers the visible area, with overflow clipped by the outer div. A CSS variable `--map-rotation` propagates the bearing down so each marker's divIcon HTML can `transform: rotate(var(--map-rotation, 0deg))` itself to keep numbers/letters/checkmarks/arrows upright in screen space. Heading is low-pass-filtered (lerp 0.3, hard-cut at >90° delta) so noisy GPS doesn't spin the map. A small N-indicator in the corner stays north-aligned. Persisted in `salary-calendar:drive:course-up`.
- After every "✓ доставлено" the route is force-rebuilt from the new GPS position to the remaining stops (rebuild fires whenever `pending` set differs from `routeRequest.stopIds`, not only when a new stop appears). This eliminates the leftover blue line going from the just-completed stop, and gives bike couriers a fresh OSRM bike route through cycleways/footways the original car-shaped detour might have missed.
- Maneuver markers on the map are limited to the next 2 *meaningful* turns (turn / merge / fork / end-of-road / on-or-off-ramp / roundabout family). Departures, arrivals and "continue straight" steps are skipped because their plain-text arrows looked like noisy random direction indicators.
- Last-stop undo: after the LAST "✓ доставлено" tap a 5-second toast "↻ отменить (Ns)" appears at the bottom of DriveMode. Tapping it removes the delivery record, resets the wave stop to pending, drops the user back into `driving` mode (which rebuilds the delivery route), and resets arrival/announcer state. Wired through `MapView.undoCompletePendingStop` → `wavesStore.undoCompleteStop` + `deliveriesStore.removeDelivery`.
...
- **Two surfaces for waves.** Floating bar over the map (`WaveTabs`) holds only the *active* wave indicator + start/finish controls and a "← к активной" button when previewing a finished one. The list of *finished* waves lives in the right sidebar's "готово" tab (`FinishedWavesList`): each card shows wave number / done-of-total / start-end times / km / earnings. Click a card → that wave is selected (`setSelectedWaveId`), its stops + saved route geometry render on the map, and inline actions appear (✕ скрыть · ↻ возобновить · удалить). Click again to deselect.
- **Map filters out finished-wave deliveries by default.** `MapView` builds a `Set<deliveryId>` from all finished waves and excludes them from the map's `deliveries` prop unless that wave is the currently selected one — so after every завершить the map cleans itself; the courier re-summons history by clicking a wave card in "готово".
...
- **Auto-finish on all-delivered.** A `useEffect` in `MapView` watches the active wave: when every stop is resolved (zero pending, ≥1 delivered) AND the user is not in DriveMode, it auto-calls `finishActiveWave`. Without this the wave stayed in an "all delivered but not finalised" limbo — invisible in готово and with its markers/route still glued to the map. Adding a new stop afterward simply spins up a fresh active wave via `ensureActiveWave`.
