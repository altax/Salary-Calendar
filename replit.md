...
- **POV chase camera** — when `followUser` is true and we have a real GPS heading, the camera goes to pitch 72°, bearing = heading (course-up), and applies bottom padding ≈ 45 % of canvas height so the user marker sits in the lower third and the road ahead dominates the frame. Falls back to plain top-down follow when no heading is available. The 3D layer is also available inside `DriveMode.tsx`: cycling the layer button now goes `карта → детальная OSM → спутник → 3D POV` and the active stop is fed in as `selectedId` so the destination building lights up while you drive.
